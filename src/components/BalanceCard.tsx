...
function SyncIndicator({ syncStatus, lastSyncedAt }: { syncStatus: SyncStatus; lastSyncedAt: string }) {
  switch (syncStatus) {
    case 'synced':
      return (
...
export function BalanceCard({ balance, syncStatus, isOptimistic }: BalanceCardProps) {
  return (
    <article
      className={`rounded-lg border bg-white p-4 shadow-sm dark:bg-zinc-900 ${
        isOptimistic ? 'border-dashed border-blue-400' : 'border-zinc-200 dark:border-zinc-700'
      }`}
    >
      <div className="mb-3 flex items-start justify-between gap-2">
        ...
        <SyncIndicator syncStatus={syncStatus} lastSyncedAt={balance.lastSyncedAt} />
      </div>

      <p className="text-3xl font-semibold text-zinc-900 dark:text-zinc-50">
        {balance.available}
        <span className="ml-1 text-base font-normal text-zinc-500">days available</span>
      </p>
      ...
      {isOptimistic && (
        <p className="mt-2 text-xs text-blue-600 dark:text-blue-400">Updating…</p>
      )}
    </article>
  );
}
